...
+ (UILabel *)createLabelWithFrame:(CGRect)frame
                         FontSize:(int)fontSize
                             Text:(NSString *)text {
    return [JDUIFactory createLabelWithFrame:frame FontSize:fontSize Text:text textColor:[UIColor blackColor]];
}
...
+ (UILabel *)createLabelWithFrame:(CGRect)frame
    ...
    return [JDUIFactory createLabelWithFrame:frame FontSize:fontSize Text:text textColor:color NSTextAlignment:NSTextAlignmentLeft];
}
...
+ (UILabel *)createLabelWithFrame:(CGRect)frame
    ...
    label.adjustsFontSizeToFitWidth = NO;                  //自适应
    label.lineBreakMode = NSLineBreakByTruncatingTail;

    label.text = text;
    return label;
}
+ (UILabel *)createBodyLabelWithFrame:(CGRect)frame
                             FontSize:(int)fontSize
                                 Text:(NSString *)text
                            textColor:(UIColor *)color
                      NSTextAlignment:(NSTextAlignment)alignment {
    ...
    label.textAlignment = alignment;                        //默认对齐方式
    label.backgroundColor = [UIColor clearColor];           //背景颜色透明
    label.font = [UIFont boldSystemFontOfSize:fontSize];    //系统字体
    label.lineBreakMode = NSLineBreakByWordWrapping;        //基准线
    label.textColor = color;                                //默认字体颜色
    label.adjustsFontSizeToFitWidth = NO;                  //自适应
    label.text = text;
